...
class FirebaseStorageDataProvider {
  ...
  static const String INCIDENT_PATH = 'incident';
  ...
  final _uploadFetcher = PublishSubject<String>();

  Stream<String> get productImageUpload => _uploadFetcher.stream;
  ...
  uploadImage(
    ...
    try {
      ...
      for (var image in images) {
        ...
        if (isIncident) {
          filePath = '$INCIDENT_IMAGES_PREFIX$fileName';

          /// FOR [incident] upload
          Reference ref = STORAGE_BUCKET.ref(INCIDENT_PATH).child(filePath);
          task = ref.putFile(image);
        }
        if (isProfile) {
          filePath = '$PROFILE_IMAGES_PREFIX$fileName';

          /// FOR [profile] upload
          Reference ref = STORAGE_BUCKET.ref(PROFILE_PATH).child(filePath);
          task = ref.putFile(image);
        }
